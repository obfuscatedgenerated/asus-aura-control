...
while True:
    ...
    screen = screen.resize((8, 8)) # Resize to a manageable size (not 1 as it is unstable)
    screen.convert("RGB") # Convert from HSV to RGB
    frame = np.array(screen) # Convert to a numpy array
    arr = frame.transpose(2, 0, 1).reshape(3, -1) # Convert frame into arrays of r, g, b
    color = arr.mean(axis=1) # Get the average color of the screen (axis 1 meaning get the average of each row)
